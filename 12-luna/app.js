...
function mapValid(string) {
    ...
    let oddValue = 0;  // нечетное значение
    for(let i = 0; i < joinNumber.length; i++){ // беру всю строку с картой и перебираю
        if(i % 2 === 0){
            if(joinNumber[i] * 2 > 9){  // получаем нечетные числа (46211356) и умножаем на 2
                evenValue += joinNumber[i] * 2 - 9; // четные числа 
            } else {
                evenValue += joinNumber[i] * 2;
            }
        } else {
            oddValue += joinNumber[i] * 1 ;
        }
    } 
...
const mapValid1 = (string) => {
    const splitNumber = string.split('-').join('').split('').map(Number)
